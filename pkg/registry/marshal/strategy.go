...
func (apiServerStrategy) Validate(ctx genericapirequest.Context, obj runtime.Object) field.ErrorList {
	...
	// return validation.ValidateFlunder(obj.(*marshal.ElasticsearchCluster))
}
...
func (apiServerStrategy) ValidateUpdate(ctx genericapirequest.Context, obj, old runtime.Object) field.ErrorList {
	return field.ErrorList{}
	// return validation.ValidateElasticsearchClusterUpdate(obj.(*marshal.ElasticsearchCluster), old.(*marshal.ElasticsearchCluster))
}

func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	apiserver, ok := obj.(*marshal.ElasticsearchCluster)
	if !ok {
		return nil, nil, fmt.Errorf("given object is not an ElasticsearchCluster.")
	}
	return labels.Set(apiserver.ObjectMeta.Labels), ElasticsearchClusterToSelectableFields(apiserver), nil
}

// MatchFlunder is the filter used by the generic etcd backend to watch events
// from etcd to clients of the apiserver only interested in specific labels/fields.
func MatchElasticsearchCluster(label labels.Selector, field fields.Selector) storage.SelectionPredicate {
...
func ElasticsearchClusterToSelectableFields(obj *marshal.ElasticsearchCluster) fields.Set {
